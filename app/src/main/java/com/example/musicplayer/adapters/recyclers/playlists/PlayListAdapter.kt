...
class PlayListAdapter (activity: Activity, lifecycleOwner: LifecycleOwner, playListName : String) :
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        /* Name TextView */
        textName.text = musicName.replace("~ Music-Fa.Com"," - ${musicArtists.replace("~ Music-Fa.Com","")} ")
        ...
        if (mainViewModel.getPlayListMusics(playListName).contains(music)) {
            Glide
                .with(activity)
                .load(liked)
                .diskCacheStrategy(DiskCacheStrategy.ALL)
